...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def get_audio(self, filename):
        ...
        # FIX VUNV: Sửa đường dẫn spec
        # VD: dataset/audio_vlsp/SPEAKER_001/SPEAKER_001_042.wav
        #  -> dataset/spec/SPEAKER_001/SPEAKER_001_042.spec.pt
        speaker_dir = os.path.dirname(filename)
        speaker_id = os.path.basename(speaker_dir)
        basename = os.path.basename(filename)
        
        spec_filename = os.path.join("dataset/spec", speaker_id, basename.replace(".wav", ".spec.pt"))
        ...
        if self.use_spk:
            ...
            # Đảm bảo dtype float32
            if spk_np.dtype != np.float32:
                spk_np = spk_np.astype(np.float32)
            spk = torch.from_numpy(spk_np)
        
        # FIX VUNV: Sửa đường dẫn SSL features (wavlm)
        # VD: dataset/audio_vlsp/SPEAKER_001/SPEAKER_001_042.wav
        #  -> dataset/wavlm/SPEAKER_001/SPEAKER_001_042.pt
        if not self.use_sr:
            c_filename = os.path.join("dataset/wavlm", speaker_id, basename.replace(".wav", ".pt"))
            
            if not os.path.exists(c_filename):
                raise FileNotFoundError(
                    f"WavLM features not found: {c_filename}\n"
                    f"Please run: python preprocess_ssl.py"
                )
            ...
            c_filename = os.path.join("dataset/sr/wavlm", speaker_id, basename.replace(".wav", f"_{i}.pt"))
            
            if not os.path.exists(c_filename):
                raise FileNotFoundError(
                    f"WavLM SR features not found: {c_filename}\n"
                    f"Please run: python preprocess_sr.py"
                )
